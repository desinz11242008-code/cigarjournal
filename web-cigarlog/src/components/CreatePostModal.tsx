    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4 backdrop-blur-sm" onClick={handleClose}>
      <div className="w-full max-w-lg rounded-3xl border border-border bg-card shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 flex flex-col gap-4">
          ...
          {/* Image Box - Slightly Smaller Height */}
          <div className="relative aspect-square w-full h-56 rounded-xl border-2 border-dashed border-border overflow-hidden bg-muted/50">
            {!imageSrc ? (
          ...
          {/* Caption Textarea - Slightly Taller Height */}
          <textarea
            value={caption}
            onChange={(e) => setCaption(e.target.value)}
            placeholder="Write a caption..."
            className="w-full h-40 p-4 rounded-xl bg-background border border-border resize-none text-sm outline-none focus:border-accent"
          />
        </div>
