    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-0 sm:p-4 backdrop-blur-sm" onClick={handleClose}>
      <div className="w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-lg sm:rounded-3xl border-0 sm:border border-border bg-card shadow-2xl flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col">
          {stage === "upload" ? (
            /* STAGE 1: Upload & Crop - Now Edge-to-Edge like Instagram */
            ...
            <div className="p-5 flex flex-col items-center gap-5">
              ...
              <div className="aspect-square w-32 sm:w-48 shrink-0 overflow-hidden rounded-xl border border-border bg-black shadow-md">
        ...
        <div className="p-4 sm:p-5 border-t border-border shrink-0 bg-card">
          {stage === "upload" ? (
            ...
            >
              <Check strokeWidth={2.5} size={18}/> Confirm Crop
            </button>
          ) : (
            <div className="flex gap-3">
                <button onClick={() => setStage("upload")} className="w-1/3 py-4 rounded-xl bg-muted text-foreground font-bold text-[15px] flex items-center justify-center gap-2 transition-transform active:scale-[0.98]">
                    <ArrowLeft strokeWidth={2.5} size={18}/> Back
                </button>
                <button 
                ...
                >
                    {isSubmitting ? <Loader2 className="animate-spin" /> : <Send strokeWidth={2.5} size={18}/>}
