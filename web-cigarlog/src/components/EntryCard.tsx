...
export function EntryCard({ entry, index }: { entry: CigarEntry; index: number }) {
  ...
  const handleShare = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevents the card from navigating to the detail page when the button is clicked
    toast.success("Ready to post! Social feed setup is next.");
    // In the next step, this will open a modal to add a caption and post to the Social feed!
  };
  ...
  return (
    ...
    >
      <div className="flex h-[130px]"> {/* Fixed height to keep cards uniform */}
